part of cobblestone;

/// Loads a font from a .fnt file and a texture
Future<BitmapFont> loadFont(String fontUrl, Future<Texture> texture) async {
  String fontData = await HttpRequest.getString(fontUrl);
  Texture atlasTexture = await texture;
  return BitmapFont.parse(fontData, atlasTexture);
}

/// Enum representing the alignment of text for [BitmapFont].
enum TextAlign {
  /// Aligns text to the left side of the given area.
  left,
  /// Aligns text to the center of the given area.
  center,
  /// Aligns text to the right side of the given area.
  right
}

/// A set of data and a texture used for rendering text.
class BitmapFont {
  // Raw data of a .fnt file
  List _data;

  // Parsed list of characters and kernings
  Map<int, _Character> _characters;
  Map<int, List<_Kerning>> _kernings;

  // The space character, used in calculations
  _Character _space;

  /// Reported font size. Characters may be taller than this.
  int size;

  /// The height of a line of text.
  int lineHeight;

  /// Distance from top of line to bottom of character.
  int base;

  /// The texture used when rendering the font.
  Texture texture;

  /// Creates a font from the contents of a .fnt file and a texture.
  BitmapFont.parse(String description, this.texture) {
    _data = _fontParser.parse(description).value;

    var info = _data.firstWhere((e) => e[0] == 'info')[1];
    size = _getVariable(info, 'size');

    var common = _data.firstWhere((e) => e[0] == 'common')[1];
    lineHeight = _getVariable(common, 'lineHeight');
    base = _getVariable(common, 'base');

    _characters = {};
    _data.where((e) => e[0] == 'char').forEach((e) {
      var props = e[1];
      _characters[_getVariable(props, 'id')] = _Character(
          _getVariable(props, 'id'),
          _getVariable(props, 'x'),
          _getVariable(props, 'y'),
          _getVariable(props, 'width'),
          _getVariable(props, 'height'),
          _getVariable(props, 'xoffset'),
          _getVariable(props, 'yoffset'),
          _getVariable(props, 'xadvance'),
          texture);
    });

    _kernings = {};
    _data.where((e) => e[0] == 'kerning').forEach((e) {
      var props = e[1];
      int second = _getVariable(props, 'second');
      _kernings.putIfAbsent(second, () => []);
      _kernings[second].add(_Kerning(_getVariable(props, 'first'),
          _getVariable(props, 'second'), _getVariable(props, 'amount')));
    });

    _space = _characters[' '.codeUnitAt(0)];
  }

  dynamic _getVariable(line, String name) {
    return line.firstWhere((e) => e[0] == name)[2];
  }

  /// Renders text to the given [batch] using the font.
  void drawParagraph(SpriteBatch batch, int x, int y, String text,
      {int lineWidth = -1, TextAlign align = TextAlign.left, double scale = 1.0}) {
    if (lineWidth == -1) {
      drawWord(batch, x, y, text);
      return;
    }

    var lines = breakLines(text, lineWidth, scale: scale);
    for (int i = 0; i < lines.length; i++) {
      drawLine(
          batch, x, (y - i * lineHeight * scale).toInt(), lines[i], lineWidth,
          align: align, scale: scale);
    }
  }

  /// Draw a line of text, usually generated by [breakLines].
  void drawLine(SpriteBatch batch, int x, int y, _Line line, int lineWidth,
      {TextAlign align = TextAlign.left, double scale = 1.0}) {
    switch (align) {
      case TextAlign.left:
        drawWord(batch, x, y, line.text);
        break;
      case TextAlign.center:
        drawWord(batch, (x + lineWidth - (line.width * scale) / 2).toInt(), y,
            line.text,
            scale: scale);
        break;
      case TextAlign.right:
        drawWord(
            batch, (x + lineWidth - line.width * scale).toInt(), y, line.text,
            scale: scale);
        break;
    }
  }

  /// Draw text without considering wrapping.
  void drawWord(SpriteBatch batch, int x, int y, String word,
      {double scale = 1.0}) {
    int lastChar = -1;

    for (var code in word.runes) {
      if (_characters.containsKey(code)) {
        x += (calcKerning(lastChar, code) * scale).toInt();
        _Character char = _characters[code];
        batch.draw(char.glyph, (x + char.xoffset * scale).toInt(),
            (y + (lineHeight - base - char.height - char.yoffset) * scale).toInt(),
            scaleX: scale, scaleY: scale);
        x += (char.xadvance * scale).toInt();
        lastChar = code;
      }
    }
  }

  /// Calculates the kerning between two characters
  int calcKerning(int last, int next) {
    int kerning = 0;
    if (_kernings.containsKey(next)) {
      _kernings[next].forEach((k) {
        if (last == k.char1) {
          kerning = k.offset;
        }
      });
    }
    return kerning;
  }

  /// Calculates the length in pixels of a string rendered with this font
  int measureWord(String word) {
    int length = 0;
    int lastChar = -1;
    for (var code in word.runes) {
      if (_characters.containsKey(code)) {
        length += _characters[code].xadvance;
        length += calcKerning(lastChar, code);
      }
    }
    return length;
  }

  /// Breaks up a word that would measure longer than [maxLength].
  Iterable<String> limitedWords(String word, int maxLength) {
    List<String> results = [];
    String current = '';
    for (var char in word.split('')) {
      current += char;
      if (measureWord(current + '-') > maxLength) {
        results.add(current.substring(0, current.length - 1) + '-');
        current = char;
      }
    }
    results.add(current);
    return results.reversed;
  }

  /// Breaks a paragraph into multiple lines, all shorter than [lineWidth].
  List<_Line> breakLines(String text, int lineWidth, {double scale = 1.0}) {
    List<_Line> lines = [];

    lineWidth = lineWidth ~/ scale;

    Queue<String> words = Queue.from(text.split(' '));

    String lineText = '';
    int cursor = 0;
    while (words.isNotEmpty) {
      String word = words.removeFirst();

      int width = measureWord(word);
      if (width > lineWidth) {
        limitedWords(word, lineWidth)
            .forEach((section) {words.addFirst(section);});
        word = words.removeFirst();
      }
      width = measureWord(word);

      if (cursor + width > lineWidth) {
        lines.add(_Line(lineText.trim(), cursor - _space.xadvance));
        cursor = 0;
        lineText = '';
      }
      lineText += word + ' ';
      cursor += width;
      cursor += _space.xadvance;
    }
    lines.add(_Line(lineText.trim(), cursor - _space.xadvance));

    return lines;
  }
}

// A line of text
class _Line {
  String text;
  int width;

  _Line(this.text, this.width);
}

// Data concerning a character in a BitmapFont
class _Character {
  int id;
  int x;
  int y;
  int width;
  int height;
  int xoffset;
  int yoffset;
  int xadvance;

  Texture font;
  Texture glyph;

  _Character(this.id, this.x, this.y, this.width, this.height, this.xoffset,
      this.yoffset, this.xadvance, this.font) {
    glyph = Texture.clone(font);
    glyph.setRegion(x, font.height - y - height, width, height);
  }
}

// Data considering the kerning between two characters
class _Kerning {
  int char1;
  int char2;

  int offset;

  _Kerning(this.char1, this.char2, this.offset);
}
